    ...
    public class PostgresDapperGenerator : DapperGenerator
    {
        protected override void GenerateGetMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                stringBuilder.AppendLine($"{PadBy(2)}const string getQuery = \"SELECT * FROM \"{table.TableName}\" WHERE {sqlWhereClauses}\";");
                stringBuilder.AppendLine($"{PadBy(2)}return await db.QuerySingleAsync<{table.DataModelName}>(getQuery, new {{ {dapperProperties} }});");
                stringBuilder.AppendLine($"{PadBy(1)}}}");
            }
            else
            {
        ...
        protected override void GenerateFindMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            stringBuilder.AppendLine($"{PadBy(1)}{{");
            stringBuilder.AppendLine($"{PadBy(2)}using var db = new NpgsqlConnection({ConnectionStringPlaceholder});");
            stringBuilder.AppendLine($"{PadBy(2)}const string findAllQuery = \"SELECT * FROM \"{table.TableName}\"\";");
            ...
            stringBuilder.AppendLine($"{PadBy(1)}{{");
            stringBuilder.AppendLine($"{PadBy(2)}using var db = new NpgsqlConnection({ConnectionStringPlaceholder});");
            stringBuilder.AppendLine($"{PadBy(2)}const string findByAllQuery = \"SELECT * FROM \"{table.TableName}\" WHERE {sqlWhereClauses}\";");
        ...
        protected override void GenerateInsertMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            var columnNames = string.Join(", ", table.Columns.Select(tc => $"\"{tc.ColumnName}\""));
            var sqlInsertValues = table.Columns.GetSqlInsertValues("@");
            var dapperParameters = table.Columns.GetDapperProperties();

            stringBuilder.AppendLine($"{PadBy(1)}public async Task<int> Create{table.TableName.ToPascalCase().RemovePluralization()}({methodParameters})");
            stringBuilder.AppendLine($"{PadBy(1)}{{");
            stringBuilder.AppendLine($"{PadBy(2)}using var db = new NpgsqlConnection({ConnectionStringPlaceholder});");
        ...
        protected override void GenerateUpdateMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                stringBuilder.AppendLine($"{PadBy(1)}public async Task<int> Update{table.TableName.ToPascalCase().RemovePluralization()}({methodParameters})");
                stringBuilder.AppendLine($"{PadBy(1)}{{");
                stringBuilder.AppendLine($"{PadBy(2)}using var db = new NpgsqlConnection({ConnectionStringPlaceholder});");
        ...
        protected override void GenerateDeleteMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                stringBuilder.AppendLine($"{PadBy(1)}public async Task<int> Delete{table.TableName.ToPascalCase().RemovePluralization()}({methodParameters})");
                stringBuilder.AppendLine($"{PadBy(1)}{{");
                stringBuilder.AppendLine($"{PadBy(2)}using var db = new NpgsqlConnection({ConnectionStringPlaceholder});");
        ...
        protected override void GenerateMergeMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                var columnNames = string.Join(", ", allColumns.Select(tc => $"\"{tc.ColumnName}\""));
                var insertValues = allColumns.GetSqlInsertValues("@");
                var conflictColumns = string.Join(", ", primaryKeyColumns.Select(c => $"\"{c.ColumnName}\""));
                var updateClauses = string.Join(", ", nonPrimaryColumns.Select(c => $"\"{c.ColumnName}\" = EXCLUDED.\"{c.ColumnName}\""));

                stringBuilder.AppendLine($"{PadBy(1)}public async Task<int> Upsert{table.TableName.ToPascalCase().RemovePluralization()}({methodParameters})");
                stringBuilder.AppendLine($"{PadBy(1)}{{");
                stringBuilder.AppendLine($"{PadBy(2)}using var db = new NpgsqlConnection({ConnectionStringPlaceholder});");

                if (nonPrimaryColumns.Any())
                {
                    stringBuilder.AppendLine($"{PadBy(2)}const string upsertQuery = \"INSERT INTO \"{table.TableName}\" ({columnNames}) VALUES ({insertValues}) ON CONFLICT ({conflictColumns}) DO UPDATE SET {updateClauses}\";");
                }
                else
                {
                    stringBuilder.AppendLine($"{PadBy(2)}const string upsertQuery = \"INSERT INTO \"{table.TableName}\" ({columnNames}) VALUES ({insertValues}) ON CONFLICT ({conflictColumns}) DO NOTHING\";");
                }
