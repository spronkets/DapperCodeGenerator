...
{
    public class MsSqlDapperGenerator : DapperGenerator
    {
        ...
        protected override void GenerateGetMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                stringBuilder.AppendLine($"{PadBy(2)}using var db = new SqlConnection({ConnectionStringPlaceholder});");
        ...
        protected override void GenerateMergeMethods(StringBuilder stringBuilder, DatabaseTable table)
        {
            ...
            if (primaryKeyColumns.Count > 0)
            {
                ...
                var sourceColumns = string.Join(", ", allColumns.Select(c => $"{GetParameterMarker()}{c.ColumnName.ToCamelCase()} AS {QuoteIdentifier(c.ColumnName)}"));
                ...
                var insertColumns = FormatColumnList(allColumns);
                ...
                stringBuilder.AppendLine($"{PadBy(1)}{{");
                stringBuilder.AppendLine($"{PadBy(2)}using var db = new SqlConnection({ConnectionStringPlaceholder});");
                stringBuilder.AppendLine($"{PadBy(2)}const string mergeQuery = @\"");
                stringBuilder.AppendLine($"{PadBy(3)}MERGE {QuoteIdentifier(table.TableName)} AS target");
                stringBuilder.AppendLine($"{PadBy(3)}USING (SELECT {sourceColumns}) AS source");
                ...
                stringBuilder.AppendLine($"{PadBy(2)}var rowsAffected = await db.ExecuteAsync(mergeQuery, new {{ {dapperProperties} }});");
                stringBuilder.AppendLine($"{PadBy(2)}return rowsAffected;");
                stringBuilder.AppendLine($"{PadBy(1)}}}");
            }
            else
